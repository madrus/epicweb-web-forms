...
We can test out the server-side validation. For that we need to temporarily disable the
client-side validation. We can do that by adding the `noValidate` attribute on the form:
...
For server-side validation errors we create an error object with the form errors and field errors:
...
To show the errors we can create the `ErrorList` component:
...
👨‍💼 Sadly, browser only validation is not enough. Users can easily change the
...
<callout-danger>
	...
	validation. Adding a `noValidate` attribute on the form allows you to submit
	...
	`maxLength`. We'll improve this next.
</callout-danger>
