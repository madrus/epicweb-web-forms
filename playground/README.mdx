...
Using Conform to validate your forms is a great way to ensure that your forms
are properly validated, both client and server side. Conform considerably
simplifies our life as it drastically reduces the amount of boilerplate code
we need to write.

Nevertheless, it was nice to first learn what is happening under the hood
of form validation in order to enjoy the simplicity of Conform optimizations.
...
build out the form in the UI. We'll use the `@conform-to/react` package for some
utilities and also a utility from `@conform-to/zod` to convert our Zod schema into
